namespace IniFileNet.Test
{
	...
	public static class ParseGood
	{
		...
		public static async Task KeyEqualsValueStream()
		{
			...
			await Chk.CheckAllIniDictionaryReader(KeyEqualsValueIni, KeyEqualsValueOpt, default, [x => Chk.CommentedValueKvp("Key", "Value", [], x)]);
		}
		...
		public static async Task DoubleKeyEqualsValueStream()
		{
			...
			c2.Next(new("Section", [new("Key", "Value1"), new("Key", "Value2")], []));
			c2.End();

			IniDictionaryReader last = new(StringComparer.OrdinalIgnoreCase);
			last.LoadOnlyLastValues(new IniStreamReader(new StringReader(DoubleKeyEqualsValueIni), DoubleKeyEqualsValueOpt)).ThrowIfError();
			Assert.Collection(last.Dictionary, [x => Chk.CommentedValueKvp("Section.Key", "Value2", [], x)]);

			IniDictionaryReader first = new(StringComparer.OrdinalIgnoreCase);
			first.LoadOnlyFirstValues(new IniStreamReader(new StringReader(DoubleKeyEqualsValueIni), DoubleKeyEqualsValueOpt)).ThrowIfError();
			Assert.Collection(first.Dictionary, [x => Chk.CommentedValueKvp("Section.Key", "Value1", [], x)]);

			IniDictionaryReader single = new(StringComparer.OrdinalIgnoreCase);
			Chk.IniError(IniErrorCode.ValueAlreadyPresent, "Section & key already present. Full Key: \"Section.Key\". Value is: \"Value2\"", single.LoadSingleValues(new IniStreamReader(new StringReader(DoubleKeyEqualsValueIni), DoubleKeyEqualsValueOpt)));
			// Even though we got an error the dictionary should still be partially filled
			Assert.Collection(single.Dictionary, [x => Chk.CommentedValueKvp("Section.Key", "Value1", [], x)]);
		}
		...
		public static async Task KeyColonValueStream()
		{
			...
			await Chk.CheckAllIniDictionaryReader(KeyColonValueIni, KeyColonValueOpt, default, [x => Chk.CommentedValueKvp("Key", "Value", [], x)]);
		}
		...
		public static async Task KeyEmptyValueStream()
		{
			...
			await Chk.CheckAllIniDictionaryReader(KeyEmptyValueIni, KeyEmptyValueOpt, default, [x => Chk.CommentedValueKvp("Key", "", [], x)]);
		}
		[Fact]
		...
		public static async Task MultipleKeyValuesStream()
		{
			...
			[
				x => Chk.CommentedValueKvp("Section.Key1", "Value1", [], x),
				x => Chk.CommentedValueKvp("Section.Key2 ", " Value2", [], x),
			]);
		}
		...
		public static async Task SectionKeyValueLineContinutationStream()
		{
			...
			[
				x => Chk.CommentedValueKvp("Section.Key1", "Helloworld!", [], x),
			]);
		}
		...
		public static async Task LineContinutationEndStream()
		{
			...
			[
				x => Chk.CommentedValueKvp("Key1", "Value", [], x),
			]);
		}
		[Fact]
		...
		public static async Task SectionCommentKeyValueStream()
		{
			...
			[
				x => Chk.CommentedValueKvp("Section.Key", "Value", [" Hello world!"], x),
			]);
		}
		...
		public static async Task CommentKeyValueStream()
		{
			...
			[
				x => Chk.CommentedValueKvp("Key", "Value", [" Hello world!"], x),
			]);
		}
		...
		public static async Task CommentIgnoredStream()
		{
			...
			[
				x => Chk.CommentedValueKvp("Key", "Value", [], x),
			]);
		}
